...
const TOP = 56;
export default class ZOPickerBar extends ZOComponent<
  ZOPickerProps,
  ZOPickerState
> {
  ...
  private getRadioRange = (begin, end): Array<string> => {
    const range: Array<string> = [];
    for (let i = begin; i <= end; i++) {
      range.push(`${i}`);
    }
  ...
  onChange(e) {
    ...
    } else if (this.props.mode === "ratio") {
      console.log("ratio");
      const range = [
        [...this.getRadioRange(0, 9)],
        [...this.getRadioRange(0, 9)],
      ];
      eventObj.detail.value = index.map((n, i) => range[i][n]).join(".");
      this.props.onChange && this.props.onChange(eventObj);
  ...
  public render(): JSX.Element {
    ...
    const getAlarmClockOrCountDown = (label = ["", ""]) => {
      const hourRange = [...this.getTimeRange(0, 23)];
      const minRange = [...this.getTimeRange(0, 59)];
      return [
        <ZOPickerView
          label={label[0]}
    ...
    // 比例
    const getRadio = (label = ["", "%"]) => {
      const range = [...this.getRadioRange(0, 9)];
      return [
        <ZOPickerView
          label={label[0]}
          onColumnChange={this.delayChange.bind(this)}
    ...
    const rootClass = classNames("zo-picker", this.props.className);
